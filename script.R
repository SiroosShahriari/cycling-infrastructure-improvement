...
{
  i <- c("2024-02-01")
  l.bike_usage_panel_df <- l.bike_usage %>% filter(date < i) %>%
...
{
  i <- c("2024-02-01")
  p.bike_usage_panel_df <- p.bike_usage %>% filter(date < i) %>%
    select(channel_id, date, value, starts_with("improvement")) %>%
    plm::pdata.frame(index = c("channel_id", "date"))
...
for (bandwidth.t in 1:3)
{
  ...
  if (bandwidth.t ==1){l.counters_spatial_weights <- GWmodel::gw.weight(l.dd,bw=bandwidth,kernel="exponential",adaptive=FALSE)}
  if (bandwidth.t ==2){l.counters_spatial_weights <- GWmodel::gw.weight(l.dd,bw=bandwidth,kernel="gaussian",adaptive=FALSE)}
  if (bandwidth.t ==3){
    listt <- c(5,10,15,20)
  ...
  if (bandwidth.t ==1){p.counters_spatial_weights <- GWmodel::gw.weight(p.dd,bw=bandwidth,kernel="exponential",adaptive=FALSE)}
  if (bandwidth.t ==2){p.counters_spatial_weights <- GWmodel::gw.weight(p.dd,bw=bandwidth,kernel="gaussian",adaptive=FALSE)}
  if (bandwidth.t ==3){
    listt <- c(5,10,15,20)
    linearbandwidth = listt[bandwidthcounter]
...
for (i in 1:15)
  ...
  l.bike_usage_panel_df <- l.bike_usage %>% filter(date <= l.i) %>%
    select(channel_id, date, value, starts_with("improvement")) %>%
    plm::pdata.frame(index = c("channel_id", "date"))

  p.bike_usage_panel_df <- p.bike_usage %>% filter(date <= p.i) %>%
    select(channel_id, date, value, starts_with("improvement")) %>%
    plm::pdata.frame(index = c("channel_id", "date"))
...
for (bandwidth in 1:4){
  ...
  for (bandwidth.t in 1:3)
  {
    ...
    if (bandwidth.t ==3){l.counters_spatial_weights <- GWmodel::gw.weight(l.dd,bw=bandwidth,kernel="exponential",adaptive=FALSE)}
    if (bandwidth.t ==2){l.counters_spatial_weights <- GWmodel::gw.weight(l.dd,bw=bandwidth,kernel="gaussian",adaptive=FALSE)}
    if (bandwidth.t ==1){
      listt <- c(5,10,15,25)
      linearbandwidth = listt[bandwidthcounter]
    ...
    if (bandwidth.t ==3){p.counters_spatial_weights <- GWmodel::gw.weight(p.dd,bw=bandwidth,kernel="exponential",adaptive=FALSE)}
    if (bandwidth.t ==2){p.counters_spatial_weights <- GWmodel::gw.weight(p.dd,bw=bandwidth,kernel="gaussian",adaptive=FALSE)}
    if (bandwidth.t ==1){
    listt <- c(5,10,15,25)
    linearbandwidth = listt[bandwidthcounter]
...
l.improvement <- l.bike_usage %>% filter(improvement == 1) %>%
...
ggplot() +
  # untreated
  geom_line(data = l.bike_usage %>% filter(!channel_id %in% l.counters_with_physical_improvement),
  ...
  guides(alpha = "none",
         # color = "none",
         size = "none") +
  theme_bw() + scale_x_date(date_break = "1 month") +
...
p.improvement <- p.bike_usage %>% filter(improvement == 1) %>%
  arrange(channel_id, date) %>%
  group_by(channel_id) %>%
  slice_head(n = 1) %>%
  select(date, channel_id, value)
...
ggplot() +
  # untreated
  geom_line(data = p.bike_usage %>% filter(!channel_id %in% p.counters_with_physical_improvement),
            aes(x = date, y = value, group = channel_id), alpha = 0.2) +
...
data <- data[-15,]
plot_ly() %>%  add_trace(data = data,  x=data$x, y=data$y, z=data$z, type="mesh3d",
                         intensity = data$z,
                         color = c(-0.004, -0.003, -0.002,-0.001),
                         colors = colorRamp(c("red", "yellow", "green")))


plot_ly() %>%  add_trace(data = data,  x=data$x, y=data$y, z=data$z, type="contour",
                         intensity = data$z,
                         color = c(-0.004, -0.003, -0.002,-0.001),
                         colors = colorRamp(c("red", "yellow", "green")))
